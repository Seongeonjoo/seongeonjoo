...
function SelectionResDetail() {
  ...
  const { data } = useSWR(
    [`/common/api/boards/${boardId}/articles/${articleId}`],
    fetcher
  );
  ...
  //목록으로 이동
  const goAnnouncementSelecton = (item: any) => {
  ...
  const download = async (attachmentId: any) => {
    fetchDownload(`http://api.bnet.com/common/api/boards/${boardId}/articles/${articleId}/attachments/${attachmentId}`);
    // fetchDownload(`http://pc.bnet.com:8081/common/api/boards/${boardId}/articles/${articleId}/attachments/${attachmentId}`);
  };
        ...
        <div className="content">
          ...
          <div css={styles.detal_list}>
            ...
            <Stack direction="row" justifyContent="center" css={styles.btnGroup}>
              <CustomButton label={'목록'} type={'small'} onClick={goAnnouncementSelecton}/>
            </Stack>
          </div>
        </div>
      </Box>
